...
class TransformerDecoder(nn.Module):
    ...
    def __call__(self, x, training: bool = True):
        ...
        pos_emb = nn.Embed(num_embeddings=self.max_len, features=self.hidden_dim)(
...
def create_train_state(
    rng: jax.random.PRNGKey, model: TransformerDecoder, learning_rate: float
) -> TrainState:
    ...
    print(param_count)
    tx = optax.adamw(learning_rate=learning_rate, b1=0.9, b2=0.98, eps=1e-9, weight_decay=0.01)
    return TrainState.create(apply_fn=model.apply, params=params, tx=tx, dropout_rng=dropout_rng)


@jax.jit
def train_step(
    state: TrainState, batch: jnp.ndarray, labels: jnp.ndarray
) -> Tuple[TrainState, jnp.ndarray]:
    """Performs a single training step."""
    dropout_rng = jax.random.fold_in(state.dropout_rng, state.step)

    def loss_fn(params):
        logits = state.apply_fn(params, batch, training=True, rngs={"dropout": dropout_rng})
...
def train_model(
    rng: jax.random.PRNGKey, train_data: jnp.ndarray, num_epochs: int = 10, batch_size: int = 1
):
